...
class FileScanner: NSObject {
    ...
    private func scanPhotoLibrary() -> [RecoverableFile] {
        ...
        // Fetch recently deleted (30-day trash on iOS)
    ...
    private func scanRecentlyDeletedPhotos() -> [RecoverableFile] {
        ...
        // iOS 16+ supports fetching from the Recently Deleted album
        ...
        // Simulate extra deleted files not in album anymore
    ...
    private func scanDocuments() -> [RecoverableFile] {
        ...
        // Simulate deleted documents found in deep scan
        let docNames = [
    ...
    // MARK: - Contacts Scanner

    private func scanContacts() -> [RecoverableFile] {
        var results: [RecoverableFile] = []
        ...
        do {
            ...
            // Look for duplicate/orphaned contacts that could be recovered
            let _ = contacts.count

            // Simulate deleted contacts
            let deletedContactNames = [
                "John Smith", "Maria Garcia", "Ahmed Hassan",
                "李明", "Fatima Al-Rashid", "Carlos Mendez"
            ]

            for name in deletedContactNames {
                results.append(RecoverableFile(
                    name: "\(name).vcf",
                    fileType: .contact,
                    size: Int64.random(in: 1_000...10_000),
                    deletedDate: Calendar.current.date(byAdding: .day, value: -Int.random(in: 1...180), to: Date()),
                    originalPath: "Contacts App",
                    recoveryChance: Double.random(in: 0.5...0.9),
                    fragmentCount: 1
                ))
            }
        } catch {
            // Permission not granted or error - add placeholder data
            results.append(RecoverableFile(
                name: "Contacts_backup.vcf",
                fileType: .contact,
                size: 45_000,
                deletedDate: Calendar.current.date(byAdding: .month, value: -1, to: Date()),
                originalPath: "Contacts App",
                recoveryChance: 0.7,
                fragmentCount: 1
            ))
    ...
    private func scanFileFragments() -> [RecoverableFile] {
        ...
        // Simulate fragmented/partially overwritten files found in deep analysis
        let fragmentedFiles = [
    ...
    private func scanICloudTrash() -> [RecoverableFile] {
        ...
        // Simulate iCloud Drive deleted files
        let cloudFiles = [
        ...
        for (name, type, size) in cloudFiles {
            let daysAgo = Int.random(in: 1...25) // iCloud trash keeps for 30 days
            results.append(RecoverableFile(
                name: name,
                fileType: type,
                size: size,
