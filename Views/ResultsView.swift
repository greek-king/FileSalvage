...
// Displays scan results with filtering and selection
...
struct ResultsView: View {
    ...
    private var summaryStrip: some View {
        HStack(spacing: 12) {
            ...
            SummaryCard(
                ...
                label: "Total Size",
                value: ByteCountFormatter.string(fromByteCount: viewModel.scanResult?.totalRecoverableSize ?? 0, countStyle: .file),
                color: "#F59E0B"
            )
        }
    }
    ...
    private var selectionToolbar: some View {
        HStack {
            Text(viewModel.selectedCount > 0
                 ? "\(viewModel.selectedCount) selected (\(ByteCountFormatter.string(fromByteCount: viewModel.selectedTotalSize, countStyle: .file)))"
    ...
    private var fileList: some View {
        ScrollView(showsIndicators: false) {
            LazyVStack(spacing: 8) {
                ...
                Spacer().frame(height: viewModel.selectedCount > 0 ? 100 : 20)
            }
            .padding(.top, 12)
        }
    }
    ...
    // MARK: - Recovery Action Bar

    private var recoveryActionBar: some View {
        VStack(spacing: 0) {
            Divider()
                .overlay(Color(hex: "#2A3352"))
            ...
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if let deletedDate = file.deletedDate {
                            Text("•")
                                .foregroundColor(Color(hex: "#2A3352"))
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .strokeBorder(
                                isSelected ? Color(hex: "#4ECDC4").opacity(0.3) : Color(hex: "#2A3352"),
                                lineWidth: 1
                            )
                    )
            )
        ...
        Button(action: action) {
            HStack(spacing: 6) {
                ...
                Text("\(count)")
                    ...
                    .background(
                        Capsule()
                            .fill(isSelected ? Color.black.opacity(0.2) : Color(hex: "#2A3352"))
                    )
            }
            ...
            .background(
                Capsule()
                    .fill(isSelected ? Color(hex: color) : Color(hex: "#141928"))
                    .overlay(
                        Capsule()
                            .strokeBorder(isSelected ? Color.clear : Color(hex: "#2A3352"), lineWidth: 1)
                    )
            )
        }
        .buttonStyle(ScaleButtonStyle())
