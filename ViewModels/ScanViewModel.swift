...
struct PermissionStatus {
    ...
    var contacts: CNAuthorizationStatus

    var allGranted: Bool {
        photos == .authorized && contacts == .authorized
    }
...
class ScanViewModel: NSObject, ObservableObject, FileScannerDelegate {
    ...
    func requestPermissions() {
        ...
        CNContactStore().requestAccess(for: .contacts) { [weak self] granted, _ in
            DispatchQueue.main.async {
                self?.permissionStatus.contacts = granted ? .authorized : .denied
            }
            group.leave()
        }

        group.notify(queue: .main) { [weak self] in
            if self?.permissionStatus.photosGranted == true {
